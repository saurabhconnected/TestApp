...
public class CustomTextField: UITextField {
    ...
    public var tfLayer:CALayer                              = CALayer()
    ...
    public var borderWidth:CGFloat                          = 0.5{
        didSet{
            let borderStyle = tfborderStyle;
            tfborderStyle = borderStyle
        }
    }
    
    public var tfborderStyle: TFBorderStyle = .rounded{
        didSet{
            borderLayer.removeFromSuperlayer()
            switch tfborderStyle {
            case .none:
                tfLayer.cornerRadius        = 0.0
                tfLayer.borderWidth         = 0.0
            case .rounded:
                tfLayer.cornerRadius        = 4.5
                tfLayer.borderWidth         = borderWidth
                tfLayer.borderColor         = borderColor.cgColor
            case .sqare:
                tfLayer.cornerRadius        = 0.0
                tfLayer.borderWidth         = borderWidth
                tfLayer.borderColor         = borderColor.cgColor
            case .bottom,.left,.right,.top:
                ...
                if tfborderStyle == .bottom {
                    borderLayer.frame = CGRect(x: 0, y: tfLayer.bounds.size.height - borderWidth, width: tfLayer.bounds.size.width, height: borderWidth)
                }else if tfborderStyle == .left{
                    borderLayer.frame = CGRect(x: 0, y: 0, width: borderWidth, height: tfLayer.bounds.size.height)
                }else if tfborderStyle == .right{
                    borderLayer.frame = CGRect(x: tfLayer.bounds.size.width - borderWidth, y: 0, width: borderWidth, height: tfLayer.bounds.size.height)
                }else{
                    borderLayer.frame = CGRect(x: 0, y: 0, width: tfLayer.bounds.size.width, height: borderWidth)
                }
                tfLayer.addSublayer(borderLayer)
            }
        }
    }
    ...
    public var borderColor: UIColor = #colorLiteral(red: 0.8, green: 0.8, blue: 0.8, alpha: 1) {
        didSet{
            switch tfborderStyle {
    ...
    public var canShowBorder:Bool = true{
        didSet{
            switch tfborderStyle {
            case .none,.rounded,.sqare:
                tfLayer.isHidden = !canShowBorder
            case .bottom,.right,.top,.left:
    ...
    fileprivate var tfLayerHeight:CGFloat{
    ...
    fileprivate func commonInit() {
        ...
        tfborderStyle               = .rounded
        tfLayer.backgroundColor     = UIColor.clear.cgColor
        
        floatPlaceholderColor       = #colorLiteral(red: 0.8, green: 0.8, blue: 0.8, alpha: 1)
        ...
        layer.insertSublayer(tfLayer, at: 0)
    }
    ...
    fileprivate func insetRectForBounds(rect:CGRect) -> CGRect {
        ...
        guard let placeholderText = lblFloatPlaceholder.text,!placeholderText.isEmpty  else {
        ...
        if floatingDisplayStatus == .never {
            ...
            if let text = text,text.isEmpty && floatingDisplayStatus == .defaults {
                return insetRectForEmptyBounds(rect: rect)
            }else{
    ...
    fileprivate func insetForSideView(forBounds bounds: CGRect) -> CGRect{
        var rect = bounds
        rect.origin.y = 0
        rect.size.height = tfLayerHeight
        return rect
    }
    ...
    override public func clearButtonRect(forBounds bounds: CGRect) -> CGRect {
        ...
        rect.origin.y = (tfLayerHeight - rect.size.height) / 2
        return rect
    }
    ...
    override public func layoutSubviews() {
        ...
        tfLayer.frame = CGRect(x: bounds.origin.x,
                               y: bounds.origin.y,
                               width: bounds.width,
                               height: tfLayerHeight)
        let borderStype = tfborderStyle
        tfborderStyle = borderStype
        CATransaction.commit()
        
        if showErrorLabel {
            
            var lblErrorFrame = lblError.frame
            lblErrorFrame.origin.y = tfLayer.frame.origin.y + tfLayer.frame.size.height + paddingYErrorLabel
